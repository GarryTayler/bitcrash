<template>
  <div class="admin-page">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="200px" align="center" label="Detail Information">
        <template slot-scope="scope">
          <span>{{ scope.row.DETAIL }}</span>
        </template>
      </el-table-column>

      <el-table-column min-width="200px" align="center" label="Destination Information">
        <template slot-scope="scope">
          <span>{{ scope.row.TXHASH }}</span>
        </template>
      </el-table-column>

      <el-table-column width="140px" align="center" label="Deposit Amount(BTC)">
        <template slot-scope="scope">
          <span>{{ parseFloat(scope.row.AMOUNT_BTC).toFixed(8) }}</span>
        </template>
      </el-table-column>
      <el-table-column width="140px" align="center" label="Deposit Amount(Coins)">
        <template slot-scope="scope">
          <span>{{ setNumberFormat(scope.row.AMOUNT_COINS) }}</span>
        </template>
      </el-table-column>
      <el-table-column width="120px" align="center" label="Fee(BTC)">
        <template slot-scope="scope">
