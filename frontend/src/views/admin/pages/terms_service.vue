...
export default {
  ...
  methods: {
    getValue() {
      ...
      getUserVariable({ 'variable': 'TERMS_AND_SERVICE' }).then(response => {
    ...
    changeFunc() {
      const loader = this.showOverlay(null)
      // updateReferralValue
      updateUserVariable({ 'variable': 'TERMS_AND_SERVICE', 'value': this.value }).then(response => {
        if (response.status === 'success') {
          this.showToast('Success', 'Terms and service is changed successfully.', 'success')
          this.hideOverlay(loader)
        } else {
          this.showToast('Error', 'Failed to change terms and service.', 'error')
          this.hideOverlay(loader)
        }
      }).catch(() => {
