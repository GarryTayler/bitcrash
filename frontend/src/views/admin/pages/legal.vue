...
export default {
  ...
  methods: {
    getValue() {
      ...
      getUserVariable({ 'variable': 'LEGAL' }).then(response => {
    ...
    changeFunc() {
      const loader = this.showOverlay(null)
      // updateReferralValue
      updateUserVariable({ 'variable': 'LEGAL', 'value': this.value }).then(response => {
        if (response.status === 'success') {
          this.showToast('Success', 'Legal is changed successfully.', 'success')
          this.hideOverlay(loader)
        } else {
          this.showToast('Error', 'Failed to change legal.', 'error')
          this.hideOverlay(loader)
        }
      }).catch(() => {
