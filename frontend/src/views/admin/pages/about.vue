...
export default {
  ...
  methods: {
    getValue() {
      ...
      getUserVariable({ 'variable': 'ABOUT' }).then(response => {
    ...
    changeFunc() {
      const loader = this.showOverlay(null)
      // updateReferralValue
      updateUserVariable({ 'variable': 'ABOUT', 'value': this.value }).then(response => {
        if (response.status === 'success') {
          this.showToast('Success', 'About is changed successfully.', 'success')
          this.hideOverlay(loader)
        } else {
          this.showToast('Error', 'Failed to change about.', 'error')
          this.hideOverlay(loader)
        }
      }).catch(() => {
