...
const typeOptions = [
  { key: 0, display_name: 'Getting started with Bitcrash' },
  { key: 1, display_name: 'Payment on Bitcrash' }
]
export default {
  components: {
  ...
  methods: {
    getValue() {
      ...
      getUserVariable({ 'variable': 'RESPONSIBLE_GAMING' }).then(response => {
    ...
    changeFunc() {
      const loader = this.showOverlay(null)
      // updateReferralValue
      updateUserVariable({ 'variable': 'RESPONSIBLE_GAMING', 'value': this.value }).then(response => {
        if (response.status === 'success') {
          this.showToast('Success', 'Responsible gaming is changed successfully.', 'success')
          this.hideOverlay(loader)
        } else {
          this.showToast('Error', 'Failed to change responsible gaming.', 'error')
          this.hideOverlay(loader)
        }
      }).catch(() => {
